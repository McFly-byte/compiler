...
//		cout << line << endl;
		...
		for ( int i = 0; i < n; i++ )
		{	
			read_next_VT( line[i], nfa, file4 );
		} 
		output( nfa.Symbol[T], nfa, file4 );
		T = nfa.Start;
		index = 0;
		word = "";
	}
	fclose(stdin);
	fout.open( file4.c_str(), ios::app );
	fout << "(" << id++ << ",#,Acc)" << endl; 
	fout.close();
}
...
//	cout << "T: " << T << "	ch: " << ch << " word: " << word << endl;
	...
	if( it == nfa.List[index].end() )
	{
		if( nfa.FinalSet[index] ) 
		{
			...
			index = 0;
			word = "";
			read_next_VT( ch, nfa, file4 ); 
		} 
		else 
		{
			output( ERROR, nfa, file4 );
		}
	}
	else {
		T = it->first;
...
void LA::output( int tag, NFA& nfa, string file4 )
{
	fout.open( file4.c_str(), ios::app );
	if( tag == ERROR )
	{
	...
	else 
	{
		...
		cout << "(" << id << ',' << word << ',' << it->first << ")" << endl; 
		fout << "(" << id++ << ',' << word << ',' << it->first << ")" << endl; 
	}
	fout.close();
}
